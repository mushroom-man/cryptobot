...
# -*- coding: utf-8 -*-
"""
Long/Short Backtest with Duration-Based Exits
===============================================
Definitive backtest comparing long-only, long/short, and long/short
with Day 50 hard exit for shorts. Includes buy-and-hold benchmark.
...
Duration Rules:
    LONG:  Hold until regime flips (no age limit)
    SHORT: Hold until regime flips OR age ≥ 50 days (hard exit)
    Age resets when position direction changes (+1 → 0/-1, -1 → 0/+1)
...
Configurations:
    ...
    B  = Long/Short — hit_rate ≤ 0.50 + sufficient → short, hold until flip
    C  = Long/Short + Day 50 — shorts hard-exit at 50 days in regime

Usage:
    python backtest_long_short_duration.py
"""
...
SHORT_MAX_AGE = 50          # Hard exit after 50 days in short regime
...
STRATEGY_CONFIGS = {
    ...
    'C': {
        ...
        'day50_exit': True,
        'is_buy_hold': False,
        'description': 'L/S + shorts hard-exit at Day 50 in regime',
    },
}
...
def generate_signals(df_1h: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Generate 16-state signals with integer state encoding."""
...
def load_all_data(db: Database) -> Dict:
    ...
    for pair in DEPLOY_PAIRS:
        ...
        signals, df_24h = generate_signals(df_1h)
...
# =============================================================================
# MAIN BACKTEST ENGINE — LONG/SHORT WITH DURATION TRACKING
# =============================================================================
...
def run_backtest(
    ...
    day50_exit: bool = False,
    use_dd_protection: bool = True,
) -> BacktestResult:
    """
    Run backtest with optional short positions and duration-based exits.
    ...
    Duration tracking:
        regime_age increments each day the position direction is unchanged
        regime_age resets to 0 when direction changes
        If day50_exit and direction == short and regime_age >= 50 → force flat
    """
    equity = INITIAL_CAPITAL
    peak_equity = INITIAL_CAPITAL
    max_drawdown = 0.0
    ...
    # Duration stats collection
    short_durations = []   # Completed short regime durations
    long_durations = []    # Completed long regime durations
    short_days = 0
    long_days = 0
    short_day50_exits = 0
    short_states_used = {}
    ...
    for i, date in enumerate(dates[:-1]):
        ...
        for pair in DEPLOY_PAIRS:
            ...
            # =============================================================
            # DURATION TRACKING & DAY 50 EXIT
            # =============================================================
            # Determine new direction from position signal
            if pos > 0:
                new_direction = 1
            elif pos < 0:
                new_direction = -1
            ...
            if new_direction == old_direction:
                # Same direction — increment age
                pair_regime_age[pair] += 1
            else:
                # Direction changed — record completed regime duration
            ...
            # Day 50 hard exit for shorts
            if day50_exit and new_direction == -1 and pair_regime_age[pair] >= SHORT_MAX_AGE:
                pos = 0.0
                short_day50_exits += 1
                # Record the completed short duration
                short_durations.append(pair_regime_age[pair])
                pair_direction[pair] = 0
                pair_regime_age[pair] = 0
    ...
    return BacktestResult(
        ...
        short_days=short_days, short_day50_exits=short_day50_exits,
...
def display_result(result: BacktestResult):
    ...
    Day 50 Forced Exits:  {result.short_day50_exits:>8,}""")
...
def display_comparison(results: Dict[str, BacktestResult]):
    ...
    LONG/SHORT PnL ATTRIBUTION:
    ──────────────────────────────────────────────────────────────────────────────────────────────────────────
    {'Config':<8s} {'Long PnL':>12s} {'L Days':>8s} {'Avg L Dur':>10s} {'Short Gross':>12s} {'S Costs':>12s} {'Short Net':>12s} {'S Days':>8s} {'Avg S Dur':>10s} {'D50 Exits':>10s}
    ──────────────────────────────────────────────────────────────────────────────────────────────────────────""")
        ...
        for config in RUN_CONFIGS:
            ...
            print(f"    {config:<8s} ${r.long_pnl:>11,.0f} {r.long_days:>8,} {r.avg_long_duration:>9.1f}d "
                  ...
                  f"{r.short_days:>8,} {r.avg_short_duration:>9.1f}d {r.short_day50_exits:>10,}")
...
def main():
    print("=" * 130)
    print("LONG/SHORT BACKTEST WITH DURATION-BASED EXITS")
    print("=" * 130)

    print(f"""
    ╔═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╗
    ║  LONG/SHORT STRATEGY WITH DURATION TRACKING                                                                             ║
    ╠═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╣
    ║                                                                                                                         ║
    ...
    ║                                                                                                                         ║
    ║  DURATION RULES:                                                                                                        ║
    ║    Longs:  Hold until regime flips (no age limit — research confirmed)                                                  ║
    ║    Shorts: Hold until regime flips OR Day {SHORT_MAX_AGE} hard exit (E[PnL] turns negative)                                     ║
    ║                                                                                                                         ║
    ║  KRAKEN COSTS:                                                                                                          ║
    ...
    ║    C  = Long/Short + Day {SHORT_MAX_AGE} (shorts hard-exit at {SHORT_MAX_AGE} days in regime)                                                     ║
    ╚═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝
    """)
    ...
    # Load data once (all configs use the same signals)
    print("\n  Loading data...")
