...
# -*- coding: utf-8 -*-
"""
Combined Strategy Backtest v5
=============================
Combines Directional 16-State and Pairs Regime Trading strategies.

16-State System (optimized from 32-state):
    - 4 price states (trend_24h × trend_168h = 2²)
    - 4 MA alignment states (ma72_above_ma24 × ma168_above_ma24 = 2²)
    - Total: 16 unique states
    
    Note: trend_72h dropped due to 80% correlation with trend_24h.
    16-state achieves better sample sufficiency and improved performance.

NEW IN v5: NO_MA72_ONLY Signal Filter
    - Skips trades when ONLY ma72_above_ma24 changed
    - Reduces turnover ~40% with minimal alpha loss
    - Research shows +0.06 Sharpe improvement

Allocation Methods:
- Primary: Risk Parity (inverse volatility weighting)
- Option: Regime Adaptive (BTC 16-State regime - CONSISTENT with trading signals)

Features:
- 16-state regime sensitivity testing
- Signal filtering for turnover reduction
- Yearly performance breakdown
- Full attribution tracking

Usage:
    python combined_backtest_with_filter.py
"""
...
# =============================================================================
# Skip trades when ONLY ma72_above_ma24 changed
# Research shows this reduces turnover ~40% with minimal alpha loss (+0.06 Sharpe)

USE_MA72_FILTER = True

# =============================================================================
# 16-STATE REGIME ALLOCATION THRESHOLDS
# =============================================================================
# Uses BTC's 16-state hit rate (same system as trading signals)
#
...
# =============================================================================
# REGIME SENSITIVITY TEST GRID (16-State Based)
# =============================================================================
...
def generate_signals(df_24h: pd.DataFrame, df_72h: pd.DataFrame,
    ...
    4 price states × 4 MA alignment states = 16 total states
    Note: trend_72h dropped due to 80% correlation with trend_24h.
...
def calculate_expanding_hit_rates(returns_history: pd.Series, 
    ...
    all_price_perms = list(product([0, 1], repeat=2))  # trend_24h, trend_168h
...
def should_trade_signal(prev_state: Optional[Tuple[int, int, int, int]], 
    ...
    """
    Determine if we should trade based on what changed.
    
    NO_MA72_ONLY filter: Skip if ONLY ma72_above_ma24 changed.
    
    State format: (trend_24h, trend_168h, ma72_above_ma24, ma168_above_ma24)
    
    Returns:
        True if we should trade (update position)
        False if we should skip (keep previous position)
    """
    if prev_state is None:
        return True
    
    if prev_state == curr_state:
        return False  # No change, no trade needed
    
    if not use_filter:
        return True  # Filter disabled, trade all changes
    ...
    # Skip if ONLY ma72_above_ma24 changed
...
def get_position(price_perm: Tuple[int, int], 
...
def load_btc_data(db: Database) -> Dict:
    """Load BTC data with 16-state signals for regime detection."""
    print("  Loading BTC data for 16-state regime detection...", flush=True)
    ...
    signals = generate_signals(btc_24h, btc_72h, btc_168h)
...
def calculate_directional_positions(data: Dict, dates: List, data_start: pd.Timestamp) -> Dict:
    ...
    for date in dates:
        for pair in DEPLOY_PAIRS:
            ...
            price_perm = (int(sig['trend_24h']), int(sig['trend_168h']))
            ...
            pos = get_position(price_perm, ma_perm, hit_rate_cache[pair])
...
def calculate_btc_hit_rate_history(btc_data: Dict, dates: List, data_start: pd.Timestamp) -> pd.Series:
    """Calculate BTC's 16-state hit rate history for regime allocation."""
    hitrate_history = {}
    hit_rate_cache = {}
...
def calculate_allocation_regime(btc_hit_rate: float,
                                         alloc_strong_buy: float,
                                         alloc_buy: float,
                                         alloc_sell: float,
                                         alloc_strong_sell: float) -> Tuple[float, float, str]:
    """
    Regime-adaptive allocation based on BTC's 16-state hit rate.
    Uses the SAME 16-state system as trading signals for consistency.
...
def run_combined_backtest(data: Dict, btc_data: Dict, btc_hitrate: pd.Series,
    ...
    """Run the combined backtest with optional signal filtering."""
    ...
    if not silent:
        filter_status = "ON" if USE_MA72_FILTER else "OFF"
        print(f"\n  Running combined backtest ({method_name}, MA72 filter: {filter_status})...", flush=True)
    ...
    for i, date in enumerate(dates[:-1]):
        ...
        if last_alloc_month != current_month:
            if use_regime:
                # Use BTC's 16-state hit rate for regime detection
                btc_hr = btc_hitrate.get(date, 0.50)
                w_dir, w_pairs, btc_state = calculate_allocation_regime(
        ...
        # ═══════════════════════════════════════════════════════════════
        # DIRECTIONAL STRATEGY (SECOND PRIORITY) - WITH SIGNAL FILTERING
        # ═══════════════════════════════════════════════════════════════
        ...
        # Calculate base exposure from signals WITH FILTERING
        base_exposure = 0.0
        asset_exposures = {}
        ...
        for pair in DEPLOY_PAIRS:
            ...
            # Check if state changed and if we should trade
            prev_state = prev_signal_states[pair]
            
            if current_state != prev_state:
                signals_total += 1
                
                if should_trade_signal(prev_state, current_state, USE_MA72_FILTER):
                    # Trade this signal - update active state
                    active_states[pair] = current_state
                else:
                    # Skip this signal - keep previous active state
                    signals_filtered += 1
            
            prev_signal_states[pair] = current_state
            
            # Use ACTIVE state for position calculation (may differ from current_state if filtered)
            active = active_states[pair]
            if active is None:
                active = current_state
                active_states[pair] = current_state
            
            price_perm = (active[0], active[1])
    ...
    if not silent:
        ...
        if USE_MA72_FILTER:
            ...
            print(f"    Signal filtering: {signals_filtered:,} / {signals_total:,} filtered ({filter_rate:.1f}%)", flush=True)
...
def display_results(rp_result: CombinedBacktestResult, 
    ...
    if USE_MA72_FILTER and rp_result.signals_total > 0:
        filter_rate = rp_result.signals_filtered / rp_result.signals_total * 100
        print(f"\n  SIGNAL FILTERING: {rp_result.signals_filtered:,} / {rp_result.signals_total:,} filtered ({filter_rate:.1f}%)")
    ...
    Trading Costs:       ${rp_result.total_trading_costs:>,.0f}
    ...
    # ─────────────────────────────────────────────────────────────
    # 16-STATE REGIME ADAPTIVE SENSITIVITY
    # ─────────────────────────────────────────────────────────────
    print("\n\n  ╔══════════════════════════════════════════════════════════════════════╗")
    print("  ║  16-STATE REGIME ADAPTIVE SENSITIVITY ANALYSIS                       ║")
    print("  ║  (Uses BTC 16-State Hit Rate - Same System as Trading Signals)       ║")
    print("  ╚══════════════════════════════════════════════════════════════════════╝")
...
def plot_results(rp_result: CombinedBacktestResult, 
                 ...
                 filename: str = 'combined_backtest_16state_filtered.png'):
    ...
    try:
        ...
        ax1.plot(best_regime.equity_curve.index, best_regime.equity_curve.values, 
                 label=f'16-State Regime ({best_regime.allocation_method})', color='#B7791F', linewidth=2)
        ax1.plot(bh_curve.index, bh_curve.values, 
                 label='Buy & Hold', color='#C53030', linewidth=2, linestyle='--')
        
        filter_label = " (MA72 Filter)" if USE_MA72_FILTER else ""
        ax1.set_title(f'Combined Strategy{filter_label} - Risk Parity vs 16-State Regime vs Buy & Hold', fontsize=12, fontweight='bold')
        ...
        ax2.set_title('Bitcoin Price (BTC/USD) - Regime from 16-State Hit Rate', fontsize=12, fontweight='bold')
        ...
        ax3.fill_between(regime_dd.index, regime_dd.values, 0, color='#B7791F', alpha=0.3, label='16-State Regime')
...
def main():
    print("=" * 80)
    print("COMBINED STRATEGY BACKTEST v5")
    print("Risk Parity + 16-State Regime Adaptive (with Signal Filtering)")
    print("=" * 80)
    
    filter_status = "ENABLED" if USE_MA72_FILTER else "DISABLED"
    print(f"""
    ╔════════════════════════════════════════════════════════════════════╗
    ║  CONFIGURATION                                                     ║
    ╠════════════════════════════════════════════════════════════════════╣
    ║  Primary: Risk Parity (inverse volatility weighting)               ║
    ║  Alternative: 16-State Regime Adaptive (BTC hit rate)              ║
    ║                                                                    ║
    ║  NEW IN v5: NO_MA72_ONLY Signal Filter ({filter_status:>8})               ║
    ║             Skips trades when ONLY ma72_above_ma24 changed         ║
    ║             Expected: ~40% turnover reduction, +0.06 Sharpe        ║
    ║                                                                    ║
    ║  16-State Allocation Grid:                                         ║
    ...
    print("  Calculating BTC 16-state hit rates...", flush=True)
    ...
    # ─────────────────────────────────────────────────────────────
    # RUN 16-STATE REGIME SENSITIVITY
    # ─────────────────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("  RUNNING 16-STATE REGIME ADAPTIVE SENSITIVITY")
    print("=" * 60)
